...
ctrdiscker = @(t,x,c) squeeze(prod(1 + (1/2)*(abs(reshape(t,[size(t,1),1,size(t,2)])-1/2) +...
    abs(reshape(x,[1,size(x,1),size(x,2)])-1/2) - ...
    abs(differ(t,x))) .* reshape(c,[1,1,size(c,2)]),3) );
...
kertitles = {'Squared Exponential Kernel','Centered Discrepancy Kernel', ...
...
xdata = (0:0.1:1)';
xcard = 4;
ecard = zeros(size(xdata)); ecard(xcard) = 1;
xx = (0:0.01:1)';
...
for jj = 1:length(kernelEx)
    ...
    figure(3*jj-1)
    kx = kernel(xplot,xdata,cVal(end));
    kxx = kernel(xdata,xdata,cVal(end));
    card = kx*(kxx\ecard);
    plot(xplot,card,'-',xdata,ecard,'.')
    xlabel('\(x\)')
    ylabel('Cardinal Function')
    title(kertitles{jj})
    print('-depsc',['Card-' kernelName{jj} '.eps'])
    ...
    xlabel('\(x_1\)')
    ylabel('\(x_2\)')
    zlabel(['\(K\bigl((x_1,x_2),(' num2str(tc2(1)) ',' num2str(tc2(2)) ') \bigr)\)'])
