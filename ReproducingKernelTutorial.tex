...
\vspace{-3ex}
Let $\cf$ \uncover<1-3>{$=$ all functions on $\{1, \ldots, d\} \text{ ``$=$'' } \reals^d$}
\hfill \uncover<4>{\alert{$\mA$ is gone.}} \hfill\phantom{a}\\
\uncover<1-3>{Pick a symmetric, positive definite (positive eigenvalues) matrix $\mA \in \reals^{d \times d}$ to define} an inner product \uncover<1-3>{\vspace{-0.3ex}
\[
\ip{f}{h} : = \vf^T \mA \vh, \quad \forall f, h \in \cf, \qquad \text{where } \vf = \bigl(f(t) \bigr)_{t=1}^d \vspace{-0.7ex}
\]
...
\begin{gather*}
    ...
    \alert{\text{Positive Definiteness }} \\
