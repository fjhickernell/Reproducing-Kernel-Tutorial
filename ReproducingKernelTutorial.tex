...
\vspace{-3ex}
Let $\cf$ \uncover<1-3>{$=$ all functions on $\{1, \ldots, d\} = \reals^d$}
\hfill \uncover<4>{\alert{$\mA$ is gone.}}\\
\uncover<1-3>{Pick a symmetric, positive definite (all eigenvalues are positive) matrix $\mA \in \reals^{d \times d}$ to define
\[
\ip{f}{h} : = \vf^T \mA \vh, \qquad \text{where } \vf = \bigl(f(t) \bigr)_{t=1}^d.
\]
